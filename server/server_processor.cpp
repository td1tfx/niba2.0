...
server_processor::server_processor(boost::asio::io_context &ioc,
    boost::asio::yield_context &yield) : ioc_(ioc), yield_(yield) {
    session_.state = nibashared::gamestate::prelogin;
}

void nibaserver::server_processor::process(nibashared::message_register &req) {
    if (db_accessor::create_user(ioc_, yield_, req.id, req.password)) {
        req.success = true;
...
void nibaserver::server_processor::process(nibashared::message_login &req) {
    if (db_accessor::login(ioc_, yield_, req.id, req.password)) {
        req.success = true;
...
void nibaserver::server_processor::process(nibashared::message_fight &req) {
    auto [self_fightable, enemy_fightable] = nibashared::prep_fight(1, 3);
